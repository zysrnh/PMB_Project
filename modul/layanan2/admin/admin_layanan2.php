<h4>Banner Link</h4>
  ...
  <a href="admin.php?pilih=layanan2&amp;modul=yes">List Data</a> | 
  <a href="admin.php?pilih=layanan2&amp;modul=yes&amp;action=add">Add Data</a> |
    <a href="admin.php?pilih=layanan2&amp;modul=yes&amp;action=cari">Cari Data</a>
...
if ($error != ''){
	...
	$insert = $koneksi_db->sql_query ("INSERT INTO `mod_data_layanan2` (`nama`,`icon`,`link`,`ket`) VALUES ('$nama','$icon','$link','$ket')");
	if ($insert) {
...
if ($error != ''){
	...
	$insert = $koneksi_db->sql_query ("UPDATE `mod_data_layanan2` SET `nama`='$nama',`icon`='$icon',`link`='$link',`ket`='$ket' WHERE `id` = '$id'");
	if ($insert) {
...
if (!isset ($_POST['submit'])){
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_layanan2` WHERE `id` = '$id'");
...
if (isset ($_POST['deleted'])){
	...
	foreach ($_POST['delete'] as $k=>$v){
		$query = $koneksi_db->sql_query ("DELETE FROM `mod_data_layanan2` WHERE `id`='$v'");
...
$num = $koneksi_db->sql_query("SELECT `id` FROM `mod_data_layanan2` $query_add");
...
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_layanan2` $query_add ORDER By `id` ASC LIMIT $offset, $limit");
...
<td>'.$data['icon'].'</td>
	...
	<td><a href="admin.php?pilih=layanan2&amp;modul=yes&amp;action=edit&amp;id='.$id.'&amp;referer='.$referer.'">Edit</a></td>
...
<input type="hidden" name="pilih" value="layanan2">
