<h4>Guru dan Karyawan</h4>
<a href="admin.php?pilih=guru&amp;modul=yes">List Data</a> | 
<a href="admin.php?pilih=guru&amp;modul=yes&amp;action=add">Add Data</a> |
<a href="admin.php?pilih=guru&amp;modul=yes&amp;action=cari">Cari Data</a>
...
<input type="hidden" name="pilih" value="guru">
...
if (isset ($_POST['deleted'])){
	...
	foreach ($_POST['delete'] as $k=>$v){
		$query = $koneksi_db->sql_query ("DELETE FROM `mod_data_guru` WHERE `id`='$v'");
	}
	}
...
$num = $koneksi_db->sql_query("SELECT `id` FROM `mod_data_guru` $query_add");
...
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_guru` $query_add $SORT_SQL LIMIT $offset, $limit");
...
$content .= '<tr>
	<td>'.$no.'.</td>
<td><img src="images/guru/'.$data['foto'].'" width="100px"></td>
		<td><b>'.$data['nama'].'</b><br/>'.$data['pekerjaan'].'<br/>Telp. '.$data['hp'].'</td>
	<td> '.$data['ket'].'</td>

	<td><a href="admin.php?pilih=guru&amp;modul=yes&amp;action=edit&amp;id='.$id.'&amp;referer='.$referer.'">Edit</a></td>
...
$content .= '<div class="error">'.$error.'</div>';
	...
	$insert = $koneksi_db->sql_query ("INSERT INTO `mod_data_guru` (`nama`,`foto`,`pekerjaan`,`hp`,`ket`,`fb`,`tw`,`in`,`nip`) VALUES ('$nama','$foto','$pekerjaan','$hp','$ket','$fb','$tw','$in','$nip')");
	if ($insert) {
...
createthumbs("images/temp/", "images/guru/thumb/", "$foto", 100);
createthumbs("images/temp/", "images/guru/", "$foto", 400);
...
$content .= '<div class="error">'.$error.'</div>';
	...
	$insert = $koneksi_db->sql_query ("UPDATE `mod_data_guru` SET `nama`='$nama',`foto`='$foto',`pekerjaan`='$pekerjaan',`hp`='$hp',`ket`='$ket',`fb`='$fb',`tw`='$tw',`in`='$in',`nip`='$nip' WHERE `id` = '$id'");
	if ($insert) {
if (!$image_name){
} else {
...
if (!isset ($_POST['submit'])){
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_guru` WHERE `id` = '$id'");
