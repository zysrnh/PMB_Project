...
$content .= '<div class="error">'.$error.'</div>';
	...
	$insert = $koneksi_db->sql_query ("INSERT INTO `mod_data_dosen` (`nama`,`foto`,`pekerjaan`,`hp`,`ket`,`fb`,`tw`,`in`) VALUES ('$nama','$foto','$pekerjaan','$hp','$ket','$fb','$tw','$in')");
	if ($insert) {
	...
	$url=str_replace(" ", "-", $nama);
		copy($_FILES['image']['tmp_name'], "./images/dosen/".$url.".jpg");
}
...
$content .= '<div class="error">'.$error.'</div>';
	...
	$insert = $koneksi_db->sql_query ("UPDATE `mod_data_dosen` SET `nama`='$nama',`foto`='$foto',`pekerjaan`='$pekerjaan',`hp`='$hp',`ket`='$ket',`fb`='$fb',`tw`='$tw',`in`='$in' WHERE `id` = '$id'");
	if ($insert) {
if (!$image_name){
} else {
	
		copy($_FILES['image']['tmp_name'], "./images/dosen/".$foto."");
}
