...
Tagihan Pembayaran
  ...
  <a href="admin.php?pilih=bayar&amp;modul=yes">List Data</a> | 
  <a href="admin.php?pilih=bayar&amp;modul=yes&amp;action=add">Add Data</a> |
  <a href="admin.php?pilih=bayar&amp;modul=yes&amp;action=cari">Cari Data</a>
...
<input type="hidden" name="pilih" value="bayar" />
...
if (isset ($_POST['deleted'])){
	...
	foreach ($_POST['delete'] as $k=>$v){
		$query = $koneksi_db->sql_query ("DELETE FROM `mod_data_bayar` WHERE `id`='$v'");
	}
	}
...
$num = $koneksi_db->sql_query("SELECT `id` FROM `mod_data_bayar` $query_add");
...
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_bayar` $query_add ORDER By `id` ASC LIMIT $offset, $limit");
...
<td>'.$data['nama'].'</td>
<td>Rp. '.matauang($data['total']).'</td>

	<td><a href="admin.php?pilih=bayar&amp;modul=yes&amp;action=edit&amp;id='.$id.'&amp;referer='.$referer.'">Edit</a></td>
...
if ($error != ''){
	...
	$insert = $koneksi_db->sql_query ("INSERT INTO `mod_data_bayar` (`nama`,`total`) VALUES ('$nama','$total')");
	if ($insert) {
...
<td>Jumlah (Rp)</td>
<td>:</td>
<td>'.input_text ('total',@$_POST['total']).'</td>
</tr>
...
if ($error != ''){
	...
	$insert = $koneksi_db->sql_query ("UPDATE `mod_data_bayar` SET `nama`='$nama',`total`='$total' WHERE `id` = '$id'");
	if ($insert) {
...
if (!isset ($_POST['submit'])){
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_bayar` WHERE `id` = '$id'");
