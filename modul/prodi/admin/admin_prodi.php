...
Data Prodi
  ...
  <a href="admin.php?pilih=prodi&amp;modul=yes">List Data</a> | 
  <a href="admin.php?pilih=prodi&amp;modul=yes&amp;action=add">Add Data</a> |
  <a href="admin.php?pilih=prodi&amp;modul=yes&amp;action=cari">Cari Data</a>
...
<input type="hidden" name="pilih" value="prodi" />
...
if (isset ($_POST['deleted'])){
	...
	foreach ($_POST['delete'] as $k=>$v){
		$query = $koneksi_db->sql_query ("DELETE FROM `mod_data_prodi` WHERE `id`='$v'");
	}
	}
...
$num = $koneksi_db->sql_query("SELECT `id` FROM `mod_data_prodi` $query_add");
...
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_prodi` $query_add ORDER By `id` ASC LIMIT $offset, $limit");
...
<td>'.$data['tampung'].'</td>
	<td><a href="admin.php?pilih=prodi&amp;modul=yes&amp;action=edit&amp;id='.$id.'&amp;referer='.$referer.'">Edit</a></td>
...
$kode = cleantext($_POST['kode']);
$nama = cleantext($_POST['nama']);
$tampung = cleantext($_POST['tampung']);
if ($error != ''){
	...
	$insert = $koneksi_db->sql_query ("INSERT INTO `mod_data_prodi` (`kode`,`nama`,`tampung`) VALUES ('$kode','$nama','$tampung')");
	if ($insert) {
...
<td>Kode</td>
<td>:</td>
<td>'.input_text ('kode',@$_POST['kode']).'</td>
</tr>
...
<td>Daya Tampung</td>
<td>:</td>
<td>'.input_text ('tampung',@$_POST['tampung']).'</td>
</tr>
...
if ($error != ''){
	...
	$insert = $koneksi_db->sql_query ("UPDATE `mod_data_prodi` SET `nama`='$nama',`tampung`='$tampung' WHERE `id` = '$id'");
	if ($insert) {
...
if (!isset ($_POST['submit'])){
$query = $koneksi_db->sql_query ("SELECT * FROM `mod_data_prodi` WHERE `id` = '$id'");
