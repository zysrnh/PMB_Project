...
$judul_clean = function_exists('clean_emoji') ? clean_emoji($data['judul']) : $data['judul'];
$konten_clean = function_exists('clean_emoji') ? clean_emoji($data['konten']) : $data['konten'];

$judulnya = $judul_clean;
$urlkontenxhal = str_replace(" ", ", ", $judulnya);

$judul_situs = ucwords($judul_clean);
$_META['description'] = limitTXT2(htmlentities(strip_tags($konten_clean)), 140);
...
if (empty($judulnya)) {
	...
	$gambar = !empty($data['foto']) && file_exists('images/' . $data['foto']) ? '<div style="margin-bottom: 25px;"><img src="images/' . $data['foto'] . '" style="max-width: 100%; height: auto; border: 1px solid #e2e8f0;" alt="' . htmlspecialchars($judul_clean) . '"></div>' : '';
    ...
    <div style="background: #f8fafc; border: 1px solid #e2e8f0; padding: 10px 18px; margin-bottom: 24px; font-size: 13px; color: #64748b; display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
        <a href="index.html" style="color: #0b4d3c; text-decoration: none; font-weight: 600;">Beranda</a>
        <i class="fa fa-angle-right" style="color: #cbd5e1;"></i>
        <span style="color: #64748b;">Program Akademik</span>
        <i class="fa fa-angle-right" style="color: #cbd5e1;"></i>
        <span style="color: #0f172a; font-weight: 600;">' . htmlspecialchars($judul_clean) . '</span>
    </div>
    ...
    <div class="hal-article-container" style="background: #ffffff; border: 1px solid #e2e8f0; border-top: 4px solid #0b4d3c; padding: 36px 40px; margin-bottom: 30px; box-shadow: 0 4px 15px rgba(0,0,0,0.03);">
        ...
        <div style="margin-top: 45px; padding: 24px; background: #f8fafc; border: 1px solid #e2e8f0; border-left: 4px solid #c89a3b; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 16px;">
            ...
            <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                ...
                <a href="https://api.whatsapp.com/send?phone=+628119081122&text=Assalamu%27alaikum%20Admin%20PMB,%20saya%20ingin%20konsultasi%20mengenai%20' . urlencode($judul_clean) . '" target="_blank" class="btn-flat btn-flat-primary" style="padding: 10px 20px; font-size: 13.5px;">
                    Kontak Helpdesk
                </a>
            </div>
        </div>
